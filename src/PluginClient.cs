...
public sealed class PluginClient : IAsyncDisposable
{
    ...
    { var values = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase); for (var i = 0; i + 1 < args.Length; i++) if (args[i].StartsWith("--", StringComparison.Ordinal)) values[args[i][2..]] = args[++i]; if (!values.TryGetValue("pipe", out var pipe) || !values.TryGetValue("plugin-id", out var id)) return null; if (!values.TryGetValue("token", out var token) && values.TryGetValue("token-file", out var tokenFile)) { token = File.ReadAllText(tokenFile).Trim(); try { File.Delete(tokenFile); } catch { } } if (string.IsNullOrWhiteSpace(token)) return null; var manifest = Path.Combine(AppContext.BaseDirectory, "plugin.json"); if (!File.Exists(manifest)) manifest = Path.Combine(AppContext.BaseDirectory, "manifest.json"); return File.Exists(manifest) ? new PluginClient(pipe, token, id, manifest) : null; }
    public async Task ConnectAsync(CancellationToken cancellationToken = default)
    {
        ...
        await SendAsync("plugin.hello", new { pluginId = _pluginId, token = _token, protocolMajor = 1, protocolMinor = 0, manifestSha256 = hash, declaredCapabilities = _capabilities, processId = Environment.ProcessId, processStartTimeUtcTicks = currentProcess.StartTime.ToUniversalTime().Ticks }, cancellationToken);
        var accepted = await ReadAsync(cancellationToken) ?? throw new InvalidDataException("Plugin host closed the handshake.");
        if (!string.Equals(accepted.Type, "host.accept", StringComparison.Ordinal)) throw new InvalidDataException("Plugin host rejected the handshake.");
    }
