...
public sealed class PluginClient : IAsyncDisposable
{
    ...
    { var values = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase); for (var i = 0; i + 1 < args.Length; i++) if (args[i].StartsWith("--", StringComparison.Ordinal)) values[args[i][2..]] = args[++i]; if (!values.TryGetValue("pipe", out var pipe) || !values.TryGetValue("token", out var token) || !values.TryGetValue("plugin-id", out var id)) return null; var manifest = Path.Combine(AppContext.BaseDirectory, "plugin.json"); if (!File.Exists(manifest)) manifest = Path.Combine(AppContext.BaseDirectory, "manifest.json"); return File.Exists(manifest) ? new PluginClient(pipe, token, id, manifest) : null; }
